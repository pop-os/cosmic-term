comment = Emulátor terminálu pro prostředí COSMIC
keywords = Příkazy;Shell;Terminál;Konzole;CLI;
settings = Nastavení
appearance = Vzhled
theme = Motiv
match-desktop = Podle systému
dark = Tmavý
light = Světlý
file = Soubor
quit = Ukončit
import = Importovat
cosmic-terminal = Terminál COSMIC
new-terminal = Nový terminál
repository = Repozitář
support = Podpora
color-schemes = Barevná schémata
rename = Přejmenovat
export = Exportovat
delete = Smazat
import-errors = Chyby importu
profiles = Profily
name = Název
command-line = Příkazový řádek
tab-title = Název karty
tab-title-description = Přepsat výchozí název karty
add-profile = Přidat profil
new-profile = Nový profil
make-default = Nastavit jako výchozí
working-directory = Pracovní adresář
hold = Podržet
remain-open = Nezavírat po ukončení podřízeného procesu.
syntax-dark = Barevné schéma tmavé
syntax-light = Barevné schéma světlé
default-zoom-step = Kroky přiblížení
opacity = Průhlednost pozadí
font = Písmo
advanced-font-settings = Pokročilá nastavení písma
default-font = Písmo
default-font-size = Velikost písma
default-font-stretch = Šířka písma
default-font-weight = Tloušťka normálního písma
default-dim-font-weight = Tloušťka zeslabeného písma
default-bold-font-weight = Tloušťka tučného písma
use-bright-bold = Světlejší tučný text
splits = Rozdělení
focus-follow-mouse = Zaměření psaní sleduje myš
advanced = Pokročilé
show-headerbar = Zobrazit záhlaví
show-header-description = Záhlaví lze odkrýt v nabídce zobrazené po kliknutí pravým tlačítkem myši
find-placeholder = Najít...
find-previous = Najít předchozí
find-next = Najít další
new-tab = Nová karta
new-window = Nové okno
profile = Profil
menu-profiles = Profily...
close-tab = Zavřít kartu
edit = Úpravy
copy = Kopírovat
paste = Vložit
select-all = Vybrat vše
find = Najít
clear-scrollback = Vymazat rolování
view = Zobrazení
zoom-in = Zvětšit text
zoom-reset = Výchozí velikost textu
zoom-out = Zmenšit text
next-tab = Další karta
previous-tab = Předchozí karta
split-horizontal = Rozdělit horizontálně
split-vertical = Rozdělit vertikálně
pane-toggle-maximize = Maximalizovat nebo obnovit podokno
menu-color-schemes = Barevná schémata...
menu-settings = Nastavení...
menu-about = O aplikaci Terminál COSMIC...
menu-password-manager = Hesla...
passwords-title = Hesla
add-password = Přidat heslo
password-input = Heslo
password-input-description = Popis
open-link = Otevřít odkaz
add-another-keybinding = Přidat další klávesovou zkratku
cancel = Zrušit
close-window = Zavřít okno
disable = Zakázat
keyboard-shortcuts = Klávesové zkratky
menu-keyboard-shortcuts = Klávesové zkratky...
no-shortcuts = Žádné zkratky
password-manager = Správce hesel
replace = Nahradit
reset-to-default = Obnovit výchozí
shortcut-group-clipboard = Schránka
shortcut-group-tabs = Karty
shortcut-group-window = Okno
shortcut-replace-title = Nahradit zkratku?
tab-activate = Aktivovat kartu { $number }
shortcut-group-other = Ostatní
shortcut-group-zoom = Přiblížení
toggle-fullscreen = Přepnout režim celé obrazovky
type-to-search = Pište pro vyhledávání...
copy-or-sigint = Kopírovat nebo SIGINT
focus-pane-down = Zaměřit podokno dole
focus-pane-left = Zaměřit podokno vlevo
focus-pane-right = Zaměřit podokno vpravo
focus-pane-up = Zaměřit podokno nahoře
shortcut-replace-body = { $binding } je již přiřazena k „{ $existing }“. Nahradit za „{ $new_action }“?
shortcut-capture-hint = Stiskněte kombinaci kláves
paste-primary = Vložit primární
copy-link = Kopírovat odkaz
tab-new-inherit-working-directory = Nové karty a okna používají aktuální adresář
tab-new-inherit-working-directory-description = Otevírat nové karty a okna v pracovním adresáři aktivní karty
show-pane-borders = Zobrazit ohraničení podoken
window-header = Záhlaví okna
